...
healthy.dat.plot <- healthy.dat[, c('age', npdr_feats[1:n_voxels])] %>%
  gather('volume', 'density', 2:(n_voxels+1)) %>%
  mutate(newvolume = gsub('srage_freesurfer_coarsevbm_', '', volume))
healthy.dat.plot$newvolume <- as.factor(healthy.dat.plot$newvolume)
levels(healthy.dat.plot$newvolume) <- paste('Voxel', 1:n_voxels)
p <- ggplot(healthy.dat.plot, aes(x = age, y = density)) +
  geom_point(alpha = 0.2, stroke = 0) + geom_smooth(color = mycolors[5]) + theme_bw() +
  facet_wrap(. ~ newvolume) +
  scale_y_continuous(breaks = scales::pretty_breaks(4)) +
  scale_x_continuous(breaks = c(25, 40, 55)) +
  labs(y = 'Gray matter density of coarse voxel', x = 'Age') +
  theme(strip.background = element_rect(fill="#fcfce6"),
        panel.spacing.x = unit(0, "lines"),
        strip.text = element_text(size = rel(0.7), margin = margin(0.5,0,0.5,0, "mm")))
p
ggsave(p, filename = 'VBMvsAge.pdf', height = 3.5, width = 3.5)
```
...
p + mytheme +
     theme(strip.text = element_text(size = rel(2.0), vjust = -4.0))
```
